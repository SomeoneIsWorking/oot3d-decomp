...
## Coverage model
SoH3D replaces an N64 actor with its OoT3D counterpart when EITHER:
  ...
  (b) The actor's room preloads an N64 object that maps to an OoT3D ZAR in
      kSoH3dObjectZars (the SOH3D_AUTO path).  This is the primary path for ~310
      of 402 objects.  Some actors (like En_Ko) load their character object via
      Object_GetIndex at runtime rather than declaring a single objectId in their
      ActorInit; the room objects list still tells us whether the requisite ZAR
      exists in that room's bank.

  (c) Special-cased actors known to be 3DS-covered via bespoke code in soh3d.c,
      regardless of sModelTable / object bank (listed in SPECIAL_COVERED below).

An actor spawn is "covered" if ANY room it appears in has at least one object that
maps to a non-NULL ZAR in kSoH3dObjectZars, OR it is in categories (a) or (c).

An actor spawn is "uncovered" (N64 fallback) if NONE of the objects preloaded for its
room have a ZAR, AND it is not in (a) or (c).
...
def analyse_scene(scene: dict, obj_zars: dict, smtable_ids: set,
                  special_ids: set, no_model_ids: set) -> dict:
    """
    ...
    # Build room_num -> room data for fast lookup
    rooms_by_num = {r["room_num"]: r for r in scene.get("rooms", [])}
    ...
    for room in scene.get("rooms", []):
        ...
        for actor in room.get("actors", []):
            ...
            if aid in no_model_ids:
                ...
                status = "covered"
                reason = f"sModelTable explicit entry"
            elif aid in special_ids:
                status = "covered"
                reason = f"special-cased in soh3d.c"
            elif rzars:
                status = "covered"
                reason = f"room objects include ZAR(s)"
            else:
                status = "uncovered"
                reason = "no ZAR for any object in this room"
            actors_out.append({
                "id": aid,
                "room": room_num,
                "pos": actor.get("pos", []),
...
def main():
    ...
    report = build_report(scenes, obj_zars, smtable_ids, special_ids, no_model_ids, actor_names)
